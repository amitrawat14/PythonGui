...
def find_rule(transport,SourceIP_var,DestinationIP_var,vsys_name,Port_var):
    ...
    vsys_command =f"set system setting target-vsys {vsysname} \n"
    vsys_command = vsys_command.encode()
    print (vsys_command)
    session.send(vsys_command)
    time.sleep(1)
    vsys_command_output = session.recv(9)
    time.sleep(1)
    print(vsys_command_output.decode())

    test_command = f'test security-policy-match source {sourceip} destination {destinationip} destination-port {port} protocol 6 \n'
    test_command_byte = test_command.encode()

    # session1 = transport.open_session()
    # session1.get_pty()
    # session1.invoke_shell() 
    # #print(test_command_byte)
#     # #test_command = f'test security-policy-match source 10.17.16.73 destination 10.168.0.0 destination-port 5985 protocol 6
    session.send(test_command_byte)
    time.sleep(3)
    x = session.recv(65535)
    time.sleep(3)
    print (f"*******************printing rule details in the Vsys {vsysname}************************************")
    x1 = x.decode()
    window1=Tk()
    scrollbar=Scrollbar(window).pack(side=RIGHT,fill=Y)
    window1.title("Welcome to Second Windows")
    window1.geometry('900x500')
    # label1=Label(window1,text=x1 ,relief="groove",bg='white',anchor='w',
    #             padx='10',justify='left',state='active',font=("arial",10)).pack(fill=BOTH)
    text = Text(window1)
...
## uncomment this option is optionbox is not wanted.
# E3_FirewallName = Entry(window, textvar=hostname_var,bd =2,width=30)
# E3_FirewallName.place( x=150, y=200)

## Creating Optionbox for selecting fw .
fw_lists = ['fw1a','fw2a','3a','4a']
